// ═══════════════════════════════════════════════════════════════════
// CONSTANTS
// ═══════════════════════════════════════════════════════════════════

const WARP_API     = 'https://api.cloudflareclient.com';
const WARP_VER     = 'v0a2158';
const SERVER_PUBKEY = 'bmXOC+F1FxEMF9dyiK2H5/1SUtzH0JuVo51h2wPfgyo=';
const DEFAULT_EP   = '162.159.192.1:2408';

const CF_RANGES = [
  { id:'r1',  label:'x.162.159.192', base:'162.159.192' },
  { id:'r2',  label:'x.162.159.193', base:'162.159.193' },
  { id:'r3',  label:'x.162.159.195', base:'162.159.195' },
  { id:'r4',  label:'x.162.159.197', base:'162.159.197' },
  { id:'r5',  label:'x.188.114.96',  base:'188.114.96'  },
  { id:'r6',  label:'x.188.114.97',  base:'188.114.97'  },
  { id:'r7',  label:'x.188.114.98',  base:'188.114.98'  },
  { id:'r8',  label:'x.188.114.99',  base:'188.114.99'  },
  { id:'r9',  label:'x.104.16.0',    base:'104.16.0'    },
  { id:'r10', label:'x.104.17.0',    base:'104.17.0'    },
  { id:'r11', label:'x.104.18.0',    base:'104.18.0'    },
  { id:'r12', label:'x.104.19.0',    base:'104.19.0'    },
  { id:'r13', label:'x.162.159.204', base:'162.159.204' },
];

const CF_PORTS = [
  { port:2408,  label:'WireGuard',      udp:true  },
  { port:443,   label:'HTTPS',          udp:false },
  { port:8443,  label:'HTTPS Alt',      udp:false },
  { port:2096,  label:'CloudFlare Alt', udp:false },
  { port:1701,  label:'L2TP',           udp:false },
  { port:500,   label:'IKE/IPsec',      udp:false },
  { port:4500,  label:'IPsec NAT-T',    udp:false },
  { port:7156,  label:'Warp Alt',       udp:false },
  { port:51820, label:'WG Default',     udp:true  },
];

// ═══════════════════════════════════════════════════════════════════
// CRYPTO
// ═══════════════════════════════════════════════════════════════════

function toB64(bytes) {
  let s = '';
  for (const b of bytes) s += String.fromCharCode(b);
  return btoa(s);
}

function toB64URL(b64) {
  return b64.replace(/\+/g,'-').replace(/\//g,'_').replace(/=/g,'');
}

async function generateKeyPair() {
  const kp    = await crypto.subtle.generateKey({ name:'X25519' }, true, ['deriveKey','deriveBits']);
  const pkcs8 = new Uint8Array(await crypto.subtle.exportKey('pkcs8', kp.privateKey));
  const pub   = new Uint8Array(await crypto.subtle.exportKey('raw',   kp.publicKey));
  return { privateKey: toB64(pkcs8.slice(-32)), publicKey: toB64(pub) };
}

// ═══════════════════════════════════════════════════════════════════
// WARP API
// ═══════════════════════════════════════════════════════════════════

async function warpRegister(publicKey) {
  const iid = crypto.randomUUID().replace(/-/g,'').slice(0,22);
  const res  = await fetch(`${WARP_API}/${WARP_VER}/reg`, {
    method: 'POST',
    headers: { 'Content-Type':'application/json', 'User-Agent':'okhttp/3.12.1' },
    body: JSON.stringify({
      key: publicKey, install_id: iid, fcm_token: '',
      tos: new Date().toISOString(), model: 'PC', serial_number: iid, locale: 'en_US',
    }),
  });
  if (!res.ok) throw new Error(`WARP register: ${res.status} — ${await res.text()}`);
  return res.json();
}

async function warpApplyLicense(deviceId, token, license) {
  const res = await fetch(`${WARP_API}/${WARP_VER}/reg/${deviceId}/account`, {
    method: 'PUT',
    headers: {
      'Content-Type':'application/json',
      'Authorization':`Bearer ${token}`,
      'User-Agent':'okhttp/3.12.1',
    },
    body: JSON.stringify({ license }),
  });
  if (!res.ok) throw new Error(`License: ${res.status} — ${await res.text()}`);
  return res.json();
}

// ═══════════════════════════════════════════════════════════════════
// CONFIG BUILDERS
// ═══════════════════════════════════════════════════════════════════

function epParts(ep) {
  const i = ep.lastIndexOf(':');
  return i > 0
    ? { host: ep.slice(0,i), port: parseInt(ep.slice(i+1)) || 2408 }
    : { host: ep, port: 2408 };
}

function buildWG(cfg, ep) {
  return `[Interface]
PrivateKey = ${cfg.privateKey}
Address = ${cfg.ipv4}/32, ${cfg.ipv6}
DNS = 1.1.1.1, 1.0.0.1, 2606:4700:4700::1111
MTU = 1280

[Peer]
PublicKey = ${cfg.serverPublicKey || SERVER_PUBKEY}
AllowedIPs = 0.0.0.0/0, ::/0
Endpoint = ${ep || cfg.endpoint || DEFAULT_EP}
PersistentKeepalive = 25
`;
}

function buildAmnezia(cfg, ep) {
  return `[Interface]
PrivateKey = ${cfg.privateKey}
Address = ${cfg.ipv4}/32, ${cfg.ipv6}
DNS = 1.1.1.1, 1.0.0.1
MTU = 1280
Jc = 120
Jmin = 23
Jmax = 47
S1 = 0
S2 = 0
H1 = 1
H2 = 2
H3 = 3
H4 = 4

[Peer]
PublicKey = ${cfg.serverPublicKey || SERVER_PUBKEY}
AllowedIPs = 0.0.0.0/0, ::/0
Endpoint = ${ep || cfg.endpoint || DEFAULT_EP}
PersistentKeepalive = 25
`;
}

function buildSingBox(cfg, ep) {
  const { host, port } = epParts(ep || cfg.endpoint || DEFAULT_EP);
  return JSON.stringify({
    outbounds: [{
      type: 'wireguard',
      tag: 'WARP',
      address: [`${cfg.ipv4}/32`, cfg.ipv6],
      private_key: cfg.privateKey,
      peers: [{
        server: host, server_port: port,
        public_key: cfg.serverPublicKey || SERVER_PUBKEY,
        allowed_ips: ['0.0.0.0/0', '::/0'],
        reserved: [0, 0, 0],
      }],
      mtu: 1280,
    }],
  }, null, 2);
}

function buildURI(cfg, ep, name = 'WARP') {
  const { host, port } = epParts(ep || cfg.endpoint || DEFAULT_EP);
  const prv  = toB64URL(cfg.privateKey);
  const pub  = toB64URL(cfg.serverPublicKey || SERVER_PUBKEY);
  const addr = encodeURIComponent(`${cfg.ipv4}/32,${cfg.ipv6}`);
  return `wireguard://${prv}@${host}:${port}?publickey=${pub}&address=${addr}&dns=1.1.1.1&mtu=1280#${encodeURIComponent(name)}`;
}

// ═══════════════════════════════════════════════════════════════════
// STORAGE
// ═══════════════════════════════════════════════════════════════════

async function kvSave(env, record) {
  if (!env.CONFIGS) return;
  await env.CONFIGS.put(`cfg:${record.id}`, JSON.stringify(record), { expirationTtl: 86400*30 });
  let list = [];
  try { list = JSON.parse(await env.CONFIGS.get('__list') || '[]'); } catch {}
  list = list.filter(x => x.id !== record.id);
  list.unshift({ id:record.id, name:record.name, accountType:record.accountType, createdAt:record.createdAt });
  await env.CONFIGS.put('__list', JSON.stringify(list.slice(0,100)));
}

async function dbSave(env, r) {
  if (!env.DB) return;
  const configText = buildWG(r, r.endpoint || DEFAULT_EP);
  await env.DB.prepare(`
    INSERT INTO configs
      (id, config_text, private_key, public_key, ipv4, ipv6, warp_account_id, created_at,
       name, device_id, device_token, server_public_key, endpoint, account_type, license_key, updated_at)
    VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)
    ON CONFLICT(id) DO UPDATE SET
      config_text=excluded.config_text,
      name=excluded.name, device_token=excluded.device_token,
      server_public_key=excluded.server_public_key,
      endpoint=excluded.endpoint, account_type=excluded.account_type,
      license_key=excluded.license_key, updated_at=excluded.updated_at
  `).bind(
    r.id, configText, r.privateKey, r.publicKey,
    r.ipv4, r.ipv6, r.deviceId||'', r.createdAt,
    r.name||'', r.deviceId||'', r.deviceToken||'',
    r.serverPublicKey||'', r.endpoint||DEFAULT_EP,
    r.accountType||'Free', r.licenseKey||'',
    r.updatedAt||r.createdAt,
  ).run();
}

async function saveConfig(env, record) {
  await Promise.all([kvSave(env, record), dbSave(env, record)]);
}

async function loadConfig(env, id) {
  if (env.CONFIGS) {
    const raw = await env.CONFIGS.get(`cfg:${id}`);
    if (raw) return JSON.parse(raw);
  }
  if (env.DB) {
    const row = await env.DB.prepare('SELECT * FROM configs WHERE id=?').bind(id).first();
    if (row) return rowToRecord(row);
  }
  return null;
}

async function listConfigs(env) {
  if (env.CONFIGS) {
    try { return JSON.parse(await env.CONFIGS.get('__list') || '[]'); } catch {}
  }
  if (env.DB) {
    const { results } = await env.DB.prepare(
      'SELECT id,name,account_type,created_at FROM configs ORDER BY created_at DESC LIMIT 100'
    ).all();
    return results.map(r => ({ id:r.id, name:r.name, accountType:r.account_type, createdAt:r.created_at }));
  }
  return [];
}

async function deleteConfig(env, id) {
  if (env.CONFIGS) {
    await env.CONFIGS.delete(`cfg:${id}`);
    let list = [];
    try { list = JSON.parse(await env.CONFIGS.get('__list') || '[]'); } catch {}
    await env.CONFIGS.put('__list', JSON.stringify(list.filter(x => x.id !== id)));
  }
  if (env.DB) await env.DB.prepare('DELETE FROM configs WHERE id=?').bind(id).run();
}

function rowToRecord(r) {
  return {
    id: r.id, name: r.name, deviceId: r.device_id, deviceToken: r.device_token,
    privateKey: r.private_key, publicKey: r.public_key, serverPublicKey: r.server_public_key,
    ipv4: r.ipv4, ipv6: r.ipv6, endpoint: r.endpoint,
    accountType: r.account_type, licenseKey: r.license_key,
    createdAt: r.created_at, updatedAt: r.updated_at,
  };
}

// ═══════════════════════════════════════════════════════════════════
// ROUTER
// ═══════════════════════════════════════════════════════════════════

const CORS = {
  'Access-Control-Allow-Origin': '*',
  'Access-Control-Allow-Methods': 'GET,POST,PUT,DELETE,OPTIONS',
  'Access-Control-Allow-Headers': 'Content-Type',
};

export default {
  async fetch(request, env) {
    const { pathname: p, searchParams } = new URL(request.url);
    const m = request.method;

    if (m === 'OPTIONS') return new Response(null, { headers: CORS });

    try {
      if (p === '/' && m === 'GET')
        return html(getSPA(CF_RANGES, CF_PORTS));

      // GET /api/configs
      if (p === '/api/configs' && m === 'GET')
        return ok(await listConfigs(env));

      // POST /api/generate
      if (p === '/api/generate' && m === 'POST') {
        const { privateKey, publicKey } = await generateKeyPair();
        const warpData = await warpRegister(publicKey);

        const res   = warpData.result ?? warpData;
        const cfg   = res.config ?? {};
        const peer  = (cfg.peers ?? [])[0] ?? {};
        const addrs = cfg.interface?.addresses ?? {};

        const list = await listConfigs(env);
        const num  = list.length + 1;
        const id   = crypto.randomUUID().replace(/-/g,'').slice(0,14);

        const record = {
          id, name: `WARP-Profile-${num} (Free)`,
          deviceId: res.id ?? '', deviceToken: res.token ?? '',
          privateKey, publicKey,
          serverPublicKey: peer.public_key ?? SERVER_PUBKEY,
          ipv4: (addrs.v4 ?? '172.16.0.2').split('/')[0],
          ipv6: addrs.v6 ?? 'fd01:5ca1:ab1e::2/128',
          endpoint: peer.endpoint?.host ?? DEFAULT_EP,
          accountType: 'Free', licenseKey: '',
          createdAt: Date.now(), updatedAt: Date.now(),
        };
        await saveConfig(env, record);
        return ok({ id, name: record.name, success: true });
      }

      // /api/config/:id/*
      const idMatch = p.match(/^\/api\/config\/([a-zA-Z0-9]+)(?:\/(.+))?$/);
      if (idMatch) {
        const [, id, sub] = idMatch;

        // DELETE
        if (!sub && m === 'DELETE') {
          await deleteConfig(env, id);
          return ok({ success: true });
        }

        // GET full record
        if (!sub && m === 'GET') {
          const rec = await loadConfig(env, id);
          return rec ? ok(rec) : err('not found', 404);
        }

        // GET config formats
        if (sub && m === 'GET') {
          const rec = await loadConfig(env, id);
          if (!rec) return err('not found', 404);
          const ep = searchParams.get('ep') || rec.endpoint || DEFAULT_EP;

          if (sub === 'wg')       return txt(buildWG(rec, ep));
          if (sub === 'amnezia')  return txt(buildAmnezia(rec, ep));
          if (sub === 'singbox')  return txt(buildSingBox(rec, ep), 'application/json');
          if (sub === 'uri')      return ok({ uri: buildURI(rec, ep, rec.name) });
          if (sub === 'download') return new Response(buildWG(rec, ep), {
            headers: { ...CORS, 'Content-Type':'text/plain',
              'Content-Disposition':`attachment; filename="warp-${id}.conf"` },
          });
        }

        // PUT endpoint
        if (sub === 'endpoint' && m === 'PUT') {
          const { endpoint } = await request.json();
          const rec = await loadConfig(env, id);
          if (!rec) return err('not found', 404);
          rec.endpoint = endpoint;
          rec.updatedAt = Date.now();
          await saveConfig(env, rec);
          return ok({ success: true, endpoint });
        }

        // PUT rename
        if (sub === 'rename' && m === 'PUT') {
          const { name } = await request.json();
          const rec = await loadConfig(env, id);
          if (!rec) return err('not found', 404);
          rec.name = name;
          rec.updatedAt = Date.now();
          await saveConfig(env, rec);
          return ok({ success: true });
        }

        // POST license
        if (sub === 'license' && m === 'POST') {
          const { license } = await request.json();
          const rec = await loadConfig(env, id);
          if (!rec) return err('not found', 404);
          if (!rec.deviceId || !rec.deviceToken) return err('device info missing', 400);
          const result = await warpApplyLicense(rec.deviceId, rec.deviceToken, license);
          const acct   = (result.result ?? result).account ?? {};
          rec.licenseKey   = license;
          rec.accountType  = acct.account_type === 'plus' ? 'Plus' : 'Free';
          rec.name         = rec.name.replace(/\((Free|Plus)\)/, `(${rec.accountType})`);
          rec.updatedAt    = Date.now();
          await saveConfig(env, rec);
          return ok({ success: true, accountType: rec.accountType, name: rec.name });
        }
      }

      return err('not found', 404);
    } catch (e) {
      console.error(e.stack ?? e.message);
      return err(e.message, 500);
    }
  },
};

const html = (body) => new Response(body, { headers: { ...CORS, 'Content-Type':'text/html;charset=utf-8' } });
const txt  = (body, ct='text/plain') => new Response(body, { headers: { ...CORS, 'Content-Type':ct } });
const ok   = (data) => new Response(JSON.stringify(data), { headers: { ...CORS, 'Content-Type':'application/json' } });
const err  = (msg, s=500) => new Response(JSON.stringify({ error: msg }), { status:s, headers:{ ...CORS,'Content-Type':'application/json' } });

// ═══════════════════════════════════════════════════════════════════
// SPA
// ═══════════════════════════════════════════════════════════════════

function getSPA(ranges, ports) {
  const R = JSON.stringify(ranges);
  const P = JSON.stringify(ports);
  return `<!DOCTYPE html>
<html lang="fa" dir="rtl">
<head>
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width,initial-scale=1">
<title>پنل WireGuard WARP</title>
<style>
:root{--bg:#0d0d1a;--bg2:#131325;--bg3:#1a1a30;--border:#252540;--accent:#f97316;--accent2:#ea580c;--blue:#3b82f6;--green:#22c55e;--red:#ef4444;--yellow:#eab308;--text:#e2e8f0;--text2:#94a3b8;--text3:#64748b;--radius:10px}
*{box-sizing:border-box;margin:0;padding:0;scrollbar-width:thin;scrollbar-color:var(--border) var(--bg)}
*::-webkit-scrollbar{width:6px;height:6px}
*::-webkit-scrollbar-track{background:var(--bg)}
*::-webkit-scrollbar-thumb{background:var(--border);border-radius:3px}
body{font-family:'Segoe UI',Tahoma,Arial,sans-serif;background:var(--bg);color:var(--text);min-height:100vh;font-size:14px}
a{color:var(--accent);text-decoration:none}

/* HEADER */
.header{background:var(--bg2);border-bottom:1px solid var(--border);padding:10px 20px;display:flex;align-items:center;justify-content:space-between;position:sticky;top:0;z-index:100}
.header-brand{display:flex;align-items:center;gap:10px}
.header-brand svg{color:var(--accent)}
.brand-title{font-size:1rem;font-weight:700;color:var(--text)}
.brand-sub{font-size:.72rem;color:var(--text3)}
.header-links{display:flex;gap:14px;align-items:center}
.header-links a{color:var(--text2);font-size:.8rem;display:flex;align-items:center;gap:4px;transition:.2s}
.header-links a:hover{color:var(--accent)}

/* LAYOUT */
.main{max-width:1000px;margin:0 auto;padding:20px 14px;display:flex;flex-direction:column;gap:16px}

/* SECTION CARD */
.card{background:var(--bg2);border:1px solid var(--border);border-radius:var(--radius);overflow:hidden}
.card-head{padding:12px 16px;border-bottom:1px solid var(--border);display:flex;align-items:center;justify-content:space-between;gap:10px}
.card-head h2{font-size:.9rem;font-weight:600;display:flex;align-items:center;gap:8px}
.card-head h2 .ico{font-size:1rem}
.card-body{padding:16px}

/* BUTTONS */
.btn{display:inline-flex;align-items:center;gap:6px;padding:8px 16px;border:none;border-radius:7px;cursor:pointer;font-size:.83rem;font-family:inherit;transition:.15s;white-space:nowrap}
.btn:disabled{opacity:.5;cursor:default}
.btn-accent{background:var(--accent);color:#fff}.btn-accent:hover:not(:disabled){background:var(--accent2)}
.btn-blue{background:var(--blue);color:#fff}.btn-blue:hover:not(:disabled){background:#2563eb}
.btn-green{background:var(--green);color:#fff}.btn-green:hover:not(:disabled){background:#16a34a}
.btn-red{background:var(--red);color:#fff}.btn-red:hover:not(:disabled){background:#dc2626}
.btn-ghost{background:transparent;border:1px solid var(--border);color:var(--text2)}.btn-ghost:hover:not(:disabled){border-color:var(--accent);color:var(--accent)}
.btn-sm{padding:5px 10px;font-size:.78rem}
.btn-xs{padding:3px 8px;font-size:.73rem}

/* INPUTS */
.inp{width:100%;padding:8px 12px;background:var(--bg3);border:1px solid var(--border);border-radius:7px;color:var(--text);font-size:.85rem;font-family:inherit;outline:none;transition:.15s}
.inp:focus{border-color:var(--accent)}
.inp-row{display:flex;gap:8px;align-items:stretch}
.inp-row .inp{flex:1}
label.lbl{display:block;font-size:.78rem;color:var(--text2);margin-bottom:4px}

/* ACCOUNT ROW */
.account-list{display:flex;flex-direction:column;gap:6px}
.account-item{display:flex;align-items:center;gap:10px;padding:10px 12px;background:var(--bg3);border:1px solid var(--border);border-radius:8px;cursor:pointer;transition:.15s}
.account-item:hover{border-color:var(--accent)}
.account-item.active{border-color:var(--accent);background:#f9731610}
.account-name{flex:1;font-size:.85rem;font-weight:500}
.badge{display:inline-block;padding:2px 8px;border-radius:10px;font-size:.68rem;font-weight:600}
.badge-free{background:#1e293b;color:var(--text2)}
.badge-plus{background:#f97316;color:#fff}
.acct-actions{display:flex;gap:6px;opacity:0;transition:.15s}
.account-item:hover .acct-actions{opacity:1}

/* ENDPOINT GRID */
.ep-grid{display:grid;grid-template-columns:1fr 1fr;gap:8px}
@media(max-width:540px){.ep-grid{grid-template-columns:1fr}}
.ep-btn{padding:8px 12px;background:var(--bg3);border:1px solid var(--border);border-radius:7px;cursor:pointer;text-align:right;transition:.15s;font-family:inherit;color:var(--text)}
.ep-btn:hover,.ep-btn.active{border-color:var(--accent);background:#f9731610;color:var(--accent)}
.ep-host{font-size:.85rem;font-weight:600}
.ep-port{font-size:.72rem;color:var(--text3);margin-top:2px}

/* TABS */
.tabs{display:flex;gap:0;border-bottom:1px solid var(--border)}
.tab{padding:9px 16px;cursor:pointer;font-size:.82rem;color:var(--text2);border-bottom:2px solid transparent;margin-bottom:-1px;transition:.15s}
.tab:hover{color:var(--text)}
.tab.active{color:var(--accent);border-bottom-color:var(--accent)}

/* CONFIG OUTPUT */
.config-box{position:relative}
.config-pre{background:var(--bg);border:1px solid var(--border);border-radius:8px;padding:14px;font-size:.78rem;color:#86efac;font-family:'Courier New',monospace;white-space:pre;overflow-x:auto;min-height:120px;line-height:1.7}
.config-actions{display:flex;gap:8px;flex-wrap:wrap;margin-top:10px}
.config-meta{margin-top:8px;font-size:.75rem;color:var(--text3)}

/* DNS/MTU */
.setting-row{display:flex;align-items:center;gap:12px;padding:8px 0}
.setting-row+.setting-row{border-top:1px solid var(--border)}
.setting-label{width:80px;font-size:.8rem;color:var(--text2);flex-shrink:0}
.dns-opts{display:flex;gap:8px;flex-wrap:wrap}
.dns-opt{padding:5px 12px;background:var(--bg3);border:1px solid var(--border);border-radius:6px;cursor:pointer;font-size:.78rem;color:var(--text2);transition:.15s}
.dns-opt.active{border-color:var(--blue);color:var(--blue);background:#3b82f610}
input[type=range]{flex:1;accent-color:var(--accent);cursor:pointer}
.range-val{min-width:36px;text-align:center;font-size:.8rem;color:var(--accent);font-weight:600}

/* SCANNER */
.scan-grid{display:grid;grid-template-columns:1fr 1fr;gap:14px}
@media(max-width:600px){.scan-grid{grid-template-columns:1fr}}
.check-list{display:flex;flex-direction:column;gap:5px;max-height:240px;overflow-y:auto}
.check-item{display:flex;align-items:center;gap:8px;padding:6px 10px;background:var(--bg3);border:1px solid var(--border);border-radius:6px;cursor:pointer;transition:.15s}
.check-item:hover{border-color:var(--accent)}
.check-item.checked{border-color:var(--accent);background:#f9731610}
.check-item input[type=checkbox]{display:none}
.check-mark{width:16px;height:16px;border:2px solid var(--border);border-radius:4px;display:flex;align-items:center;justify-content:center;flex-shrink:0;transition:.15s}
.check-item.checked .check-mark{background:var(--accent);border-color:var(--accent)}
.check-mark::after{content:'✓';color:#fff;font-size:10px;display:none}
.check-item.checked .check-mark::after{display:block}
.check-text{font-size:.8rem;flex:1}
.check-sub{font-size:.7rem;color:var(--text3)}
.scan-controls{display:flex;gap:8px;align-items:center;flex-wrap:wrap;margin-top:12px}
.sel-btns{display:flex;gap:6px}

/* SCAN RESULTS TABLE */
.results-wrap{overflow-x:auto;margin-top:14px}
.results-table{width:100%;border-collapse:collapse;font-size:.8rem}
.results-table th{background:var(--bg3);padding:8px 12px;text-align:right;color:var(--text2);font-weight:600;border-bottom:2px solid var(--border)}
.results-table td{padding:8px 12px;border-bottom:1px solid var(--border)}
.results-table tr:hover td{background:#ffffff08}
.latency-bar{display:flex;align-items:center;gap:6px}
.latency-num{min-width:48px;font-weight:600}
.latency-bg{flex:1;background:var(--bg3);border-radius:4px;height:6px;overflow:hidden}
.latency-fill{height:100%;border-radius:4px;transition:.3s}
.lat-good{color:var(--green)}.lat-mid{color:var(--yellow)}.lat-bad{color:var(--red)}
.status-dot{width:8px;height:8px;border-radius:50%;display:inline-block}
.dot-ok{background:var(--green)}.dot-err{background:var(--red)}.dot-scan{background:var(--yellow);animation:pulse 1s infinite}
@keyframes pulse{0%,100%{opacity:1}50%{opacity:.4}}
.scan-progress{margin-top:10px;background:var(--bg3);border-radius:6px;height:6px;overflow:hidden}
.scan-progress-fill{height:100%;background:var(--accent);border-radius:6px;transition:.3s}

/* QR MODAL */
.modal-overlay{display:none;position:fixed;inset:0;background:#00000088;z-index:200;align-items:center;justify-content:center}
.modal-overlay.show{display:flex}
.modal{background:var(--bg2);border:1px solid var(--border);border-radius:12px;padding:24px;max-width:320px;width:90%;text-align:center}
.modal h3{font-size:.95rem;margin-bottom:16px;color:var(--text)}
#qr-canvas{background:#fff;padding:10px;border-radius:8px;display:inline-block}
.modal-sub{font-size:.75rem;color:var(--text3);margin-top:10px}

/* SPINNER */
.spin{display:inline-block;width:14px;height:14px;border:2px solid currentColor;border-top-color:transparent;border-radius:50%;animation:spin .6s linear infinite;vertical-align:middle}
@keyframes spin{to{transform:rotate(360deg)}}

/* TOAST */
.toast{position:fixed;bottom:20px;left:50%;transform:translateX(-50%);background:#1e293b;color:var(--text);padding:10px 20px;border-radius:8px;font-size:.83rem;z-index:300;border:1px solid var(--border);transition:.3s;opacity:0;pointer-events:none}
.toast.show{opacity:1}

/* MISC */
.empty{text-align:center;color:var(--text3);padding:24px;font-size:.85rem}
.info-badge{display:inline-flex;align-items:center;gap:4px;padding:3px 8px;background:var(--bg3);border:1px solid var(--border);border-radius:6px;font-size:.75rem;color:var(--text2)}
.divider{border:none;border-top:1px solid var(--border);margin:12px 0}
.row{display:flex;gap:8px;align-items:center;flex-wrap:wrap}
.hidden{display:none!important}
</style>
</head>
<body>

<div class="header">
  <div class="header-brand">
    <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
      <path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/>
    </svg>
    <div>
      <div class="brand-title">پنل کانفیگ وایرگارد کلادفلر WARP</div>
      <div class="brand-sub">تولید سریع و مدیریت کانفیگ‌های وایرگارد با استفاده از کلادفلر</div>
    </div>
  </div>
  <div class="header-links">
    <a href="https://github.com/ScannerVpn/wireguard-cloudflare" target="_blank">
      <svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor"><path d="M12 0C5.37 0 0 5.37 0 12c0 5.31 3.44 9.8 8.2 11.39.6.11.82-.26.82-.58l-.01-2.03c-3.34.72-4.04-1.61-4.04-1.61-.55-1.39-1.34-1.76-1.34-1.76-1.09-.75.08-.73.08-.73 1.2.08 1.84 1.24 1.84 1.24 1.07 1.83 2.8 1.3 3.49 1 .1-.78.42-1.3.76-1.6-2.67-.3-5.47-1.33-5.47-5.93 0-1.31.47-2.38 1.24-3.22-.13-.3-.54-1.52.12-3.18 0 0 1.01-.32 3.3 1.23A11.5 11.5 0 0 1 12 5.8c1.02 0 2.04.14 3 .4 2.28-1.55 3.29-1.23 3.29-1.23.66 1.66.25 2.88.12 3.18.77.84 1.24 1.91 1.24 3.22 0 4.61-2.81 5.63-5.48 5.92.43.37.81 1.1.81 2.22l-.01 3.29c0 .32.21.7.82.58C20.56 21.8 24 17.31 24 12c0-6.63-5.37-12-12-12z"/></svg>
      GitHub
    </a>
    <a href="#" onclick="toggleDir();return false" id="dirBtn">English</a>
  </div>
</div>

<div class="main">

  <!-- ACCOUNT MANAGEMENT -->
  <div class="card">
    <div class="card-head">
      <h2><span class="ico">👤</span> مدیریت حساب‌ها</h2>
      <div class="row">
        <button class="btn btn-accent" onclick="generateAccount()">
          <span id="genSpin" class="spin hidden"></span>
          <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/></svg>
          تولید اکانت جدید
        </button>
        <button class="btn btn-ghost" onclick="showImportDialog()">
          <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M21 15v4a2 2 0 01-2 2H5a2 2 0 01-2-2v-4"/><polyline points="7 10 12 15 17 10"/><line x1="12" y1="15" x2="12" y2="3"/></svg>
          وارد کردن اکانت
        </button>
      </div>
    </div>
    <div class="card-body">
      <div id="accountList" class="account-list">
        <div class="empty">در حال بارگذاری…</div>
      </div>
    </div>
  </div>

  <!-- WARP+ LICENSE -->
  <div class="card" id="licenseCard">
    <div class="card-head">
      <h2><span class="ico">⭐</span> ارتقا به WARP Plus</h2>
      <span class="info-badge" id="acctTypeBadge">انتخاب حساب</span>
    </div>
    <div class="card-body">
      <label class="lbl">دریافت لایسنس پلاس:</label>
      <div class="inp-row">
        <input class="inp" id="licenseInput" placeholder="XXXXX-XXXXX-XXXXX" style="direction:ltr;letter-spacing:1px">
        <button class="btn btn-accent" onclick="applyLicense()">
          <span id="licSpin" class="spin hidden"></span>
          اعمال لایسنس
        </button>
      </div>
      <p style="font-size:.75rem;color:var(--text3);margin-top:6px">لایسنس WARP+ را از اپ Cloudflare 1.1.1.1 دریافت کنید (Referral)</p>
    </div>
  </div>

  <!-- ENDPOINT SELECTION -->
  <div class="card">
    <div class="card-head">
      <h2><span class="ico">📍</span> انتخاب نقطه پایانی (Endpoint)</h2>
      <span class="info-badge" id="curEpBadge">—</span>
    </div>
    <div class="card-body">
      <p style="font-size:.78rem;color:var(--text3);margin-bottom:12px">نقطه پایانی فعال برای استفاده در تنظیمات:</p>
      <div class="ep-grid" id="epGrid"></div>
      <hr class="divider">
      <label class="lbl">آی‌پی / Endpoint سفارشی:</label>
      <div class="inp-row">
        <input class="inp" id="customEp" placeholder="162.159.192.1:2408" style="direction:ltr">
        <button class="btn btn-blue" onclick="applyCustomEp()">
          <span id="epSpin" class="spin hidden"></span>
          اعمال آی‌پی
        </button>
      </div>
      <p style="font-size:.75rem;color:var(--text3);margin-top:6px">آی‌پی‌های پیشنهادی کلادفلر</p>
    </div>
  </div>

  <!-- DNS & MTU -->
  <div class="card">
    <div class="card-head">
      <h2><span class="ico">⚙️</span> تنظیمات پیشرفته (DNS & MTU)</h2>
    </div>
    <div class="card-body">
      <div class="setting-row">
        <span class="setting-label">سرور DNS</span>
        <div class="dns-opts">
          <div class="dns-opt active" data-dns="1.1.1.1, 1.0.0.1, 2606:4700:4700::1111" onclick="setDns(this)">Cloudflare (1.1.1.1)</div>
          <div class="dns-opt" data-dns="8.8.8.8, 8.8.4.4" onclick="setDns(this)">Google (8.8.8.8)</div>
          <div class="dns-opt" data-dns="9.9.9.9, 149.112.112.112" onclick="setDns(this)">Quad9 (9.9.9.9)</div>
          <div class="dns-opt" data-dns="94.140.14.14, 94.140.15.15" onclick="setDns(this)">AdGuard</div>
        </div>
      </div>
      <div class="setting-row">
        <span class="setting-label">MTU Size</span>
        <input type="range" min="1000" max="1500" value="1280" step="10" id="mtuSlider" oninput="document.getElementById('mtuVal').textContent=this.value">
        <span class="range-val" id="mtuVal">1280</span>
      </div>
    </div>
  </div>

  <!-- CONFIG OUTPUT -->
  <div class="card">
    <div class="card-head">
      <h2><span class="ico">🔑</span> کانفیگ‌های آماده شده</h2>
      <span class="info-badge" id="configStatus">بدون انتخاب</span>
    </div>
    <div>
      <div class="tabs" id="fmtTabs">
        <div class="tab active" data-fmt="wg"      onclick="switchFmt(this)">WireGuard Standard</div>
        <div class="tab"       data-fmt="amnezia"  onclick="switchFmt(this)">AmneziaWG (DPI Bypass)</div>
        <div class="tab"       data-fmt="singbox"  onclick="switchFmt(this)">Sing-Box JSON</div>
        <div class="tab"       data-fmt="sub"      onclick="switchFmt(this)">ساب‌اسکریپشن / URI</div>
      </div>
      <div class="card-body">
        <div id="noConfigMsg" class="empty">ابتدا یک حساب انتخاب یا تولید کنید</div>
        <div id="configOutput" class="config-box hidden">
          <pre class="config-pre" id="configPre">—</pre>
          <div class="config-actions">
            <button class="btn btn-green btn-sm" onclick="copyConfig()">📋 کپی</button>
            <button class="btn btn-blue btn-sm"  onclick="downloadConfig()">⬇ دانلود .conf</button>
            <button class="btn btn-ghost btn-sm" onclick="showQR()">📱 QR کد</button>
            <button class="btn btn-ghost btn-sm" onclick="copyURI()">🔗 لینک اشتراک</button>
          </div>
          <div class="config-meta" id="configMeta"></div>
        </div>
      </div>
    </div>
  </div>

  <!-- IP SCANNER -->
  <div class="card">
    <div class="card-head">
      <h2><span class="ico">🔍</span> اسکن آی‌پی تمیز کلادفلر</h2>
    </div>
    <div class="card-body">
      <div class="scan-grid">
        <!-- IP Ranges -->
        <div>
          <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px">
            <span style="font-size:.8rem;color:var(--text2);font-weight:600">انتخاب رنج‌های آی‌پی کلادفلر:</span>
            <div class="sel-btns">
              <button class="btn btn-ghost btn-xs" onclick="selectAllRanges(true)">انتخاب همه</button>
              <button class="btn btn-ghost btn-xs" onclick="selectAllRanges(false)">حذف همه</button>
            </div>
          </div>
          <div class="check-list" id="rangeList"></div>
        </div>
        <!-- Ports -->
        <div>
          <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px">
            <span style="font-size:.8rem;color:var(--text2);font-weight:600">انتخاب پورت‌ها:</span>
            <div class="sel-btns">
              <button class="btn btn-ghost btn-xs" onclick="selectAllPorts(true)">انتخاب همه</button>
              <button class="btn btn-ghost btn-xs" onclick="selectAllPorts(false)">حذف همه</button>
            </div>
          </div>
          <div class="check-list" id="portList"></div>
        </div>
      </div>

      <div class="setting-row" style="margin-top:12px">
        <span class="setting-label" style="width:120px">تعداد آی‌پی هر رنج:</span>
        <input type="range" min="1" max="50" value="15" id="ipCountSlider" oninput="document.getElementById('ipCountVal').textContent=this.value">
        <span class="range-val" id="ipCountVal">15</span>
      </div>

      <div class="scan-controls">
        <button class="btn btn-accent" id="scanBtn" onclick="startScan()">
          <span id="scanSpin" class="spin hidden"></span>
          🚀 شروع فرایند اسکن پیشرفته
        </button>
        <button class="btn btn-ghost btn-sm" onclick="clearResults()">پاک کردن نتایج</button>
        <span id="scanStats" style="font-size:.78rem;color:var(--text3)"></span>
      </div>

      <div class="scan-progress hidden" id="scanProgressWrap">
        <div class="scan-progress-fill" id="scanProgressFill" style="width:0%"></div>
      </div>

      <div class="results-wrap" id="resultsWrap">
        <div class="empty" id="noResults">هیچ آی‌پی فعالی روی این رنج‌ها کشف نشده است.</div>
        <table class="results-table hidden" id="resultsTable">
          <thead>
            <tr>
              <th></th>
              <th>آدرس آی‌پی تمیز</th>
              <th>پورت</th>
              <th>تاخیر</th>
              <th>وضعیت</th>
              <th>عملیات</th>
            </tr>
          </thead>
          <tbody id="resultsBody"></tbody>
        </table>
      </div>
    </div>
  </div>

</div><!-- /main -->

<!-- QR MODAL -->
<div class="modal-overlay" id="qrModal" onclick="closeQR(event)">
  <div class="modal">
    <h3>اسکن کد QR کانفیگ</h3>
    <div id="qr-canvas"></div>
    <div class="modal-sub" id="qrSub"></div>
    <div style="margin-top:14px;display:flex;gap:8px;justify-content:center">
      <button class="btn btn-ghost btn-sm" id="qrAmnezia" onclick="qrToggle('amnezia')">Amnezia</button>
      <button class="btn btn-ghost btn-sm" id="qrStd" onclick="qrToggle('standard')">Standard</button>
      <button class="btn btn-red btn-sm" onclick="closeQR()">بستن</button>
    </div>
  </div>
</div>

<!-- TOAST -->
<div class="toast" id="toast"></div>

<script>
// ─── DATA ────────────────────────────────────────────────────────────────────
const CF_RANGES = ${R};
const CF_PORTS  = ${P};

let state = {
  configs: [],
  current: null,
  fmt: 'wg',
  ep: '',
  dns: '1.1.1.1, 1.0.0.1, 2606:4700:4700::1111',
  mtu: 1280,
  selectedRanges: new Set(CF_RANGES.map(r => r.id)),
  selectedPorts:  new Set([443, 8443, 2096]),
  scanResults: [],
  scanning: false,
  scanAbort: null,
};

// ─── INIT ─────────────────────────────────────────────────────────────────────
(async () => {
  buildRangeList();
  buildPortList();
  buildEpGrid();
  await loadConfigs();
  if (state.configs.length > 0) await selectConfig(state.configs[0].id);
})();

// ─── CONFIGS ──────────────────────────────────────────────────────────────────
async function loadConfigs() {
  const data = await api('/api/configs');
  state.configs = data;
  renderAccountList();
}

function renderAccountList() {
  const el = document.getElementById('accountList');
  if (!state.configs.length) {
    el.innerHTML = '<div class="empty">هیچ حسابی یافت نشد — یک حساب جدید تولید کنید</div>';
    return;
  }
  el.innerHTML = state.configs.map(c => \`
    <div class="account-item \${state.current?.id === c.id ? 'active' : ''}" onclick="selectConfig('\${c.id}')">
      <div class="account-name">\${esc(c.name)}</div>
      <span class="badge \${c.accountType === 'Plus' ? 'badge-plus' : 'badge-free'}">\${c.accountType}</span>
      <div class="acct-actions" onclick="event.stopPropagation()">
        <button class="btn btn-ghost btn-xs" onclick="renameConfig('\${c.id}','\${esc(c.name)}')">✏️</button>
        <button class="btn btn-ghost btn-xs" onclick="deleteConfig('\${c.id}')">🗑</button>
      </div>
    </div>
  \`).join('');
}

async function generateAccount() {
  const spin = document.getElementById('genSpin');
  spin.classList.remove('hidden');
  try {
    const data = await api('/api/generate', 'POST');
    if (data.error) return toast('خطا: ' + data.error, true);
    toast('✅ حساب جدید ساخته شد: ' + data.name);
    await loadConfigs();
    await selectConfig(data.id);
  } catch(e) { toast('خطا: ' + e.message, true); }
  finally { spin.classList.add('hidden'); }
}

async function selectConfig(id) {
  const rec = await api(\`/api/config/\${id}\`);
  if (rec.error) return;
  state.current = rec;
  state.ep = rec.endpoint || '162.159.192.1:2408';
  document.getElementById('curEpBadge').textContent = state.ep;
  document.getElementById('acctTypeBadge').textContent =
    rec.accountType === 'Plus' ? '⭐ WARP Plus' : 'رایگان (Free)';
  renderAccountList();
  updateEpGrid();
  await renderConfig();
}

async function deleteConfig(id) {
  if (!confirm('این حساب حذف شود؟')) return;
  await api(\`/api/config/\${id}\`, 'DELETE');
  if (state.current?.id === id) {
    state.current = null;
    document.getElementById('configOutput').classList.add('hidden');
    document.getElementById('noConfigMsg').classList.remove('hidden');
  }
  await loadConfigs();
  toast('حساب حذف شد');
}

async function renameConfig(id, oldName) {
  const name = prompt('نام جدید:', oldName);
  if (!name || name === oldName) return;
  await api(\`/api/config/\${id}/rename\`, 'PUT', { name });
  await loadConfigs();
  if (state.current?.id === id) state.current.name = name;
  toast('نام تغییر کرد');
}

function showImportDialog() {
  toast('قابلیت ورود مستقیم کانفیگ به زودی اضافه می‌شود');
}

// ─── LICENSE ──────────────────────────────────────────────────────────────────
async function applyLicense() {
  if (!state.current) return toast('ابتدا یک حساب انتخاب کنید', true);
  const key = document.getElementById('licenseInput').value.trim();
  if (!key) return toast('لایسنس را وارد کنید', true);
  const spin = document.getElementById('licSpin');
  spin.classList.remove('hidden');
  try {
    const res = await api(\`/api/config/\${state.current.id}/license\`, 'POST', { license: key });
    if (res.error) return toast('خطا: ' + res.error, true);
    state.current.accountType = res.accountType;
    state.current.name        = res.name;
    document.getElementById('acctTypeBadge').textContent =
      res.accountType === 'Plus' ? '⭐ WARP Plus' : 'رایگان (Free)';
    await loadConfigs();
    toast('✅ لایسنس اعمال شد — ' + res.accountType);
  } catch(e) { toast('خطا: ' + e.message, true); }
  finally { spin.classList.add('hidden'); }
}

// ─── ENDPOINT ─────────────────────────────────────────────────────────────────
const PRESETS = [
  { host:'162.159.192.1', port:2408 },
  { host:'162.159.193.1', port:2408 },
  { host:'162.159.195.1', port:2408 },
  { host:'188.114.96.1',  port:2408 },
  { host:'162.159.192.1', port:443  },
  { host:'162.159.193.1', port:443  },
  { host:'162.159.192.1', port:8443 },
  { host:'188.114.96.1',  port:2096 },
];

function buildEpGrid() {
  document.getElementById('epGrid').innerHTML = PRESETS.map(ep => \`
    <button class="ep-btn" onclick="applyPresetEp('\${ep.host}:\${ep.port}')">
      <div class="ep-host">\${ep.host}</div>
      <div class="ep-port">:\${ep.port}</div>
    </button>
  \`).join('');
}

function updateEpGrid() {
  document.querySelectorAll('.ep-btn').forEach(b => {
    const ep = b.querySelector('.ep-host').textContent + ':' + b.querySelector('.ep-port').textContent.slice(1);
    b.classList.toggle('active', ep === state.ep);
  });
  document.getElementById('customEp').value = state.ep;
}

async function applyPresetEp(ep) { await doApplyEp(ep); }

async function applyCustomEp() {
  const val = document.getElementById('customEp').value.trim();
  if (!val) return;
  await doApplyEp(val);
}

async function doApplyEp(ep) {
  if (!state.current) return toast('ابتدا یک حساب انتخاب کنید', true);
  const spin = document.getElementById('epSpin');
  spin.classList.remove('hidden');
  try {
    await api(\`/api/config/\${state.current.id}/endpoint\`, 'PUT', { endpoint: ep });
    state.ep = ep;
    state.current.endpoint = ep;
    document.getElementById('curEpBadge').textContent = ep;
    updateEpGrid();
    await renderConfig();
    toast('Endpoint به‌روز شد');
  } catch(e) { toast('خطا: ' + e.message, true); }
  finally { spin.classList.add('hidden'); }
}

// ─── DNS / MTU ───────────────────────────────────────────────────────────────
function setDns(el) {
  document.querySelectorAll('.dns-opt').forEach(d => d.classList.remove('active'));
  el.classList.add('active');
  state.dns = el.dataset.dns;
  renderConfig();
}

document.getElementById('mtuSlider').addEventListener('input', function() {
  state.mtu = parseInt(this.value);
  renderConfig();
});

// ─── CONFIG OUTPUT ────────────────────────────────────────────────────────────
function switchFmt(el) {
  document.querySelectorAll('.tab').forEach(t => t.classList.remove('active'));
  el.classList.add('active');
  state.fmt = el.dataset.fmt;
  renderConfig();
}

async function renderConfig() {
  if (!state.current) return;
  const pre    = document.getElementById('configPre');
  const out    = document.getElementById('configOutput');
  const noMsg  = document.getElementById('noConfigMsg');
  const meta   = document.getElementById('configMeta');
  const status = document.getElementById('configStatus');

  out.classList.remove('hidden');
  noMsg.classList.add('hidden');
  status.textContent = state.current.name;

  const ep  = state.ep || state.current.endpoint || '162.159.192.1:2408';
  const fmt = state.fmt;

  if (fmt === 'sub') {
    const res = await api(\`/api/config/\${state.current.id}/uri?ep=\${encodeURIComponent(ep)}\`);
    pre.textContent = res.uri || '';
    meta.textContent = '⚡ این لینک را در اپ‌های v2rayNG، Shadowrocket یا Amnezia ایمپورت کنید';
    return;
  }

  const url = \`/api/config/\${state.current.id}/\${fmt}?ep=\${encodeURIComponent(ep)}\`;
  const txt = await fetch(url).then(r => r.text());
  // Patch DNS and MTU dynamically
  let out2 = txt
    .replace(/^DNS = .*/m, \`DNS = \${state.dns}\`)
    .replace(/^MTU = .*/m, \`MTU = \${state.mtu}\`);
  pre.textContent = out2;
  meta.textContent = \`IPv4: \${state.current.ipv4}  |  IPv6: \${(state.current.ipv6||'').split('/')[0]}  |  Endpoint: \${ep}\`;
}

async function copyConfig() {
  await navigator.clipboard.writeText(document.getElementById('configPre').textContent);
  toast('✅ کپی شد');
}

async function copyURI() {
  if (!state.current) return;
  const ep  = state.ep || state.current.endpoint || '162.159.192.1:2408';
  const res = await api(\`/api/config/\${state.current.id}/uri?ep=\${encodeURIComponent(ep)}\`);
  await navigator.clipboard.writeText(res.uri || '');
  toast('✅ لینک اشتراک کپی شد');
}

async function downloadConfig() {
  if (!state.current) return;
  const ep  = state.ep || state.current.endpoint || '162.159.192.1:2408';
  const url = \`/api/config/\${state.current.id}/download?ep=\${encodeURIComponent(ep)}\`;
  const a   = document.createElement('a');
  a.href     = url;
  a.download = \`warp-\${state.current.id}.conf\`;
  a.click();
}

// ─── QR CODE ─────────────────────────────────────────────────────────────────
let qrMode = 'standard';

async function showQR() {
  if (!state.current) return toast('ابتدا یک حساب انتخاب کنید', true);
  document.getElementById('qrModal').classList.add('show');
  await renderQR();
}

async function renderQR() {
  const ep  = state.ep || state.current.endpoint || '162.159.192.1:2408';
  let   text;
  if (qrMode === 'amnezia') {
    text = await fetch(\`/api/config/\${state.current.id}/amnezia?ep=\${encodeURIComponent(ep)}\`).then(r => r.text());
    document.getElementById('qrSub').textContent = 'AmneziaWG — برای اپ Amnezia';
  } else {
    text = await fetch(\`/api/config/\${state.current.id}/wg?ep=\${encodeURIComponent(ep)}\`).then(r => r.text());
    document.getElementById('qrSub').textContent = 'WireGuard Standard';
  }
  const el = document.getElementById('qr-canvas');
  el.innerHTML = '';
  try {
    new QRCode(el, { text, width:220, height:220, correctLevel:QRCode.CorrectLevel.M });
  } catch {
    el.innerHTML = '<p style="color:#f00;font-size:.8rem">خطا در ساخت QR — متن خیلی طولانی است</p>';
  }
}

function qrToggle(mode) {
  qrMode = mode;
  document.getElementById('qrAmnezia').classList.toggle('btn-accent', mode === 'amnezia');
  document.getElementById('qrStd').classList.toggle('btn-accent', mode === 'standard');
  renderQR();
}

function closeQR(e) {
  if (!e || e.target === document.getElementById('qrModal')) {
    document.getElementById('qrModal').classList.remove('show');
  }
}

// ─── SCANNER ──────────────────────────────────────────────────────────────────
function buildRangeList() {
  document.getElementById('rangeList').innerHTML = CF_RANGES.map(r => \`
    <label class="check-item \${state.selectedRanges.has(r.id) ? 'checked' : ''}" id="range_\${r.id}" onclick="toggleRange('\${r.id}')">
      <div class="check-mark"></div>
      <div class="check-text">\${r.label}</div>
    </label>
  \`).join('');
}

function buildPortList() {
  document.getElementById('portList').innerHTML = CF_PORTS.map(p => \`
    <label class="check-item \${state.selectedPorts.has(p.port) ? 'checked' : ''}" id="port_\${p.port}" onclick="togglePort(\${p.port})">
      <div class="check-mark"></div>
      <div class="check-text">
        <span>\${p.label}</span>
        <span class="check-sub" style="float:left;direction:ltr">\${p.port} \${p.udp ? '(UDP)' : '(TCP)'}</span>
      </div>
    </label>
  \`).join('');
}

function toggleRange(id) {
  state.selectedRanges.has(id) ? state.selectedRanges.delete(id) : state.selectedRanges.add(id);
  document.getElementById('range_' + id)?.classList.toggle('checked', state.selectedRanges.has(id));
}

function togglePort(port) {
  state.selectedPorts.has(port) ? state.selectedPorts.delete(port) : state.selectedPorts.add(port);
  document.getElementById('port_' + port)?.classList.toggle('checked', state.selectedPorts.has(port));
}

function selectAllRanges(v) {
  CF_RANGES.forEach(r => {
    v ? state.selectedRanges.add(r.id) : state.selectedRanges.delete(r.id);
    document.getElementById('range_' + r.id)?.classList.toggle('checked', v);
  });
}

function selectAllPorts(v) {
  CF_PORTS.forEach(p => {
    v ? state.selectedPorts.add(p.port) : state.selectedPorts.delete(p.port);
    document.getElementById('port_' + p.port)?.classList.toggle('checked', v);
  });
}

async function startScan() {
  if (state.scanning) {
    state.scanAbort?.abort();
    state.scanning = false;
    document.getElementById('scanBtn').innerHTML = '🚀 شروع فرایند اسکن پیشرفته';
    document.getElementById('scanSpin').classList.add('hidden');
    document.getElementById('scanProgressWrap').classList.add('hidden');
    return;
  }

  const ranges     = CF_RANGES.filter(r => state.selectedRanges.has(r.id));
  const ports      = CF_PORTS.filter(p => state.selectedPorts.has(p.port));
  const ipsPerRange = parseInt(document.getElementById('ipCountSlider').value);

  if (!ranges.length || !ports.length) return toast('رنج یا پورت انتخاب نشده', true);

  // Generate random IPs
  const tasks = [];
  for (const range of ranges) {
    const octets = new Set();
    while (octets.size < ipsPerRange) octets.add(Math.floor(Math.random()*254)+1);
    for (const oct of octets) {
      const ip = range.base + '.' + oct;
      for (const p of ports) tasks.push({ ip, port: p.port, udp: p.udp, label: p.label });
    }
  }

  state.scanning  = true;
  state.scanAbort = new AbortController();
  state.scanResults = [];

  document.getElementById('scanBtn').innerHTML = '⏹ توقف اسکن';
  document.getElementById('scanSpin').classList.remove('hidden');
  document.getElementById('scanProgressWrap').classList.remove('hidden');
  document.getElementById('scanProgressFill').style.width = '0%';
  document.getElementById('noResults').classList.add('hidden');
  document.getElementById('resultsTable').classList.remove('hidden');
  document.getElementById('resultsBody').innerHTML = '';

  let done = 0;
  const CONCURRENCY = 20;
  const queue = [...tasks];

  async function runWorker() {
    while (queue.length && !state.scanAbort.signal.aborted) {
      const task = queue.shift();
      if (!task) break;
      const result = await testIP(task.ip, task.port, task.udp, state.scanAbort.signal);
      state.scanResults.push(result);
      done++;
      const pct = Math.round(done / tasks.length * 100);
      document.getElementById('scanProgressFill').style.width = pct + '%';
      document.getElementById('scanStats').textContent = \`\${done}/\${tasks.length} — یافت شده: \${state.scanResults.filter(r=>r.ok).length}\`;
      if (result.ok || result.latency < 5000) addResultRow(result);
    }
  }

  const workers = Array.from({ length: CONCURRENCY }, runWorker);
  await Promise.all(workers);

  state.scanning = false;
  document.getElementById('scanBtn').innerHTML = '🚀 شروع فرایند اسکن پیشرفته';
  document.getElementById('scanSpin').classList.add('hidden');
  document.getElementById('scanProgressWrap').classList.add('hidden');

  sortResults();
  toast('✅ اسکن تمام شد — ' + state.scanResults.filter(r=>r.ok).length + ' آی‌پی یافت شد');
}

async function testIP(ip, port, isUDP, signal) {
  if (isUDP) {
    // UDP cannot be tested from browser; simulate with HTTPS on same IP
    return await testHTTPS(ip, 443, signal).then(r => ({ ...r, port, udp: true, simulated: true }));
  }
  return testHTTPS(ip, port, signal);
}

async function testHTTPS(ip, port, signal) {
  const https  = [443, 8443, 2096].includes(port);
  const proto  = https ? 'https' : 'http';
  const url    = \`\${proto}://\${ip}:\${port}/cdn-cgi/trace\`;
  const start  = performance.now();
  try {
    await fetch(url, { mode:'no-cors', cache:'no-store', signal: AbortSignal.any([signal, AbortSignal.timeout(3000)]) });
    return { ip, port, latency: Math.round(performance.now()-start), ok:true };
  } catch(e) {
    const lat = Math.round(performance.now()-start);
    return { ip, port, latency: e.name==='AbortError' ? 9999 : lat, ok: lat < 3000 && e.name !== 'AbortError' };
  }
}

function addResultRow(r) {
  const latClass = r.latency < 100 ? 'lat-good' : r.latency < 300 ? 'lat-mid' : 'lat-bad';
  const maxLat   = 1000;
  const barW     = Math.min(100, Math.round(r.latency / maxLat * 100));
  const barColor = r.latency < 100 ? '#22c55e' : r.latency < 300 ? '#eab308' : '#ef4444';
  const tbody    = document.getElementById('resultsBody');
  const tr       = document.createElement('tr');
  tr.innerHTML = \`
    <td><span class="status-dot \${r.ok?'dot-ok':'dot-err'}"></span></td>
    <td style="font-family:monospace;direction:ltr">\${r.ip}</td>
    <td style="direction:ltr">\${r.port}\${r.udp?' (UDP*)':''}</td>
    <td>
      <div class="latency-bar">
        <span class="latency-num \${latClass}">\${r.latency >= 9999 ? 'timeout' : r.latency+'ms'}</span>
        <div class="latency-bg"><div class="latency-fill" style="width:\${barW}%;background:\${barColor}"></div></div>
      </div>
    </td>
    <td>\${r.ok ? '<span style="color:var(--green)">✓ فعال</span>' : '<span style="color:var(--red)">✗ خطا</span>'}</td>
    <td>
      \${r.ok ? \`<button class="btn btn-blue btn-xs" onclick="applyScannedIP('\${r.ip}',\${r.port})">اعمال</button>\` : '—'}
    </td>
  \`;
  tbody.appendChild(tr);
}

function sortResults() {
  const tbody = document.getElementById('resultsBody');
  const rows  = Array.from(tbody.querySelectorAll('tr'));
  rows.sort((a,b) => {
    const la = parseInt(a.querySelector('.latency-num')?.textContent) || 9999;
    const lb = parseInt(b.querySelector('.latency-num')?.textContent) || 9999;
    return la - lb;
  });
  rows.forEach(r => tbody.appendChild(r));
}

async function applyScannedIP(ip, port) {
  const ep = \`\${ip}:\${port}\`;
  document.getElementById('customEp').value = ep;
  await doApplyEp(ep);
  toast(\`✅ آی‌پی \${ep} اعمال شد\`);
}

function clearResults() {
  state.scanResults = [];
  document.getElementById('resultsBody').innerHTML = '';
  document.getElementById('resultsTable').classList.add('hidden');
  document.getElementById('noResults').classList.remove('hidden');
  document.getElementById('scanStats').textContent = '';
}

// ─── UTILS ───────────────────────────────────────────────────────────────────
async function api(url, method='GET', body) {
  const opts = { method, headers:{'Content-Type':'application/json'} };
  if (body) opts.body = JSON.stringify(body);
  const res = await fetch(url, opts);
  return res.json();
}

function esc(s) { return String(s).replace(/[&<>"]/g, c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;'}[c])); }

function toast(msg, isErr=false) {
  const t = document.getElementById('toast');
  t.textContent = msg;
  t.style.background = isErr ? '#7f1d1d' : '#1e293b';
  t.classList.add('show');
  setTimeout(() => t.classList.remove('show'), 2800);
}

function toggleDir() {
  const html = document.documentElement;
  const isRTL = html.dir === 'rtl';
  html.dir = isRTL ? 'ltr' : 'rtl';
  document.getElementById('dirBtn').textContent = isRTL ? 'فارسی' : 'English';
}

if (!window.AbortSignal?.any) {
  AbortSignal.any = (signals) => {
    const ctrl = new AbortController();
    signals.forEach(s => s.addEventListener('abort', () => ctrl.abort()));
    return ctrl.signal;
  };
}
</script>
<script src="https://cdnjs.cloudflare.com/ajax/libs/qrcodejs/1.0.0/qrcode.min.js" crossorigin="anonymous"></script>
</body>
</html>`;
}
